import{s as d}from"./supabase-B4HPT40D.js";async function m(n,t){var l;n.innerHTML='<div class="spinner"></div>';let i=50,e=0;try{const{count:s}=await d.from("mint_queue").select("*",{count:"exact",head:!0});e=s||0}catch{}const r=Math.round(e/i*100),c=i-e;if(t.address,n.innerHTML=`
    <div class="card-glow" style="text-align:center;padding:1.5rem 1rem">
      ...
      <div style="margin-bottom:0.5rem">
        <div style="display:flex;justify-content:space-between;font-size:0.75rem;font-weight:700;margin-bottom:0.35rem">
          <span style="color:var(--accent)">${e} minted</span>
          <span style="color:var(--text3)">${c} remaining</span>
        </div>
        <div style="background:var(--surface2);border-radius:6px;height:8px;overflow:hidden">
          <div style="height:100%;width:${r}%;background:linear-gradient(90deg,var(--accent),var(--purple));border-radius:6px;transition:width 0.5s"></div>
        </div>
        <div style="font-size:0.68rem;color:var(--text3);margin-top:0.3rem">${r}% claimed</div>
      </div>
    </div>
  ...
  `,(l=document.getElementById("mint-connect"))==null||l.addEventListener("click",()=>{var s;(s=document.getElementById("auth-badge"))==null||s.click()}),t.address)try{const{data:s}=await d.from("mint_queue").select("member_number,spore_id,status").eq("ckb_address",t.address).single(),a=document.getElementById("mint-status");if(!a)return;s!=null&&s.spore_id?a.innerHTML=`
          <div class="stat-row"><span class="stat-label">Member #</span><span class="stat-value accent">#${s.member_number}</span></div>
          <div class="stat-row"><span class="stat-label">DOB Minted</span><span class="stat-value green">✓ Yes</span></div>
          <div class="stat-row"><span class="stat-label">Spore ID</span><span class="stat-value" style="font-size:0.64rem">${s.spore_id.slice(0,22)}…</span></div>
        `:s?a.innerHTML=`
          <div class="stat-row"><span class="stat-label">Member #</span><span class="stat-value accent">#${s.member_number}</span></div>
          <div class="stat-row"><span class="stat-label">DOB Status</span><span class="stat-value orange">Pending mint</span></div>
        `:a.innerHTML='<div style="color:var(--text3);font-size:0.82rem;padding:0.5rem 0">Not yet registered — sign up at wyltekindustries.com</div>'}catch{}}export{m as renderMint};
